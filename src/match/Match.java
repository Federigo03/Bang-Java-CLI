...
public class Match {
    ...
    public Match(int nStartPlayers, boolean inputName){
        ...
        while (match) {
            ...
            if(turnPlayer.isActiveCard("Jail")){
                ...
                boolean res = turnPlayer.drawHearts(Deck, DiscardPile);
            ...
            switch(turnPlayer.getCharacter().getName()){
                case "Jesse Jones":
                    ...
                    do{
                        choice = Utils.nextInt(turnPlayer + ": <Choose 1 to activate your character's ability or 0 to ignore");
                    }while(choice < 0 || choice > 1);
                    if(choice == 1){
                        do{
                            printNHands(0);
                            choice = Utils.nextInt(turnPlayer + ": <Choose the player you want to draw from or 0 to cancel") - 1;
                ...
                case "Kit Carlson":
                    sidKetchum(turnPlayer + " can activate his ability", DiscardPile);
                    turnPlayer.kitCarlson(Deck, DiscardPile);
                    //drawn = 2;    useless but attention at the next if
                    break;
                case "Pedro Ramirez":
                    readDiscard(DiscardPile);
                    sidKetchum(turnPlayer + " can activate his ability", DiscardPile);
                    if(turnPlayer.pedroRamirez(DiscardPile))
                        drawn++;
            }
            if(turnPlayer.getCharacter().getName() == "Kit Carlson"){
                sidKetchum(turnPlayer + " is going to draw", DiscardPile);
                turnPlayer.firstPhase(Deck, DiscardPile, drawn);
            }
            ...
            while(goon){
                ...
                switch (choice) {
                    ...
                    case 2:
                        do{
                            printActivePlayers();
                            choice = Utils.nextInt(turnPlayer + ": <Choose player's id to check his character or 0 to check everyone's");
                        }while(choice < 0 || choice > nPlayers);
                    ...
                    case 4:
                        do{
                            printActivePlayers();
                            choice = Utils.nextInt(turnPlayer + ": <Choose player's id to check the number of cards in his hands or 0 to check everyone's");
                        }while(choice < 0 || choice > nPlayers);
                        printNHands(choice);
                        break;
                    case 5:
                    ...
                    case 7:
                        do{
                            printActivePlayers();
                            choice = Utils.nextInt(turnPlayer + ": <Choose player's id to check his active cards or 0 to check everyones'");
                        }while(choice < 0 || choice > nPlayers);
                    ...
                    case 8:
                        do{
                            printActivePlayers();
                            choice = Utils.nextInt(turnPlayer + ": <Choose player's id to check his active range or 0 to check everyones'");
                        }while(choice < 0 || choice > nPlayers);
                        printRange(choice);
                        break;
                    case 9:
                        do{
                            printActivePlayers();
                            choice = Utils.nextInt(turnPlayer + ": <Choose player's id to check his actual distance from you or 0 to check everyone's");
                        }while(choice < 0 || choice > nPlayers);
                    ...
                    case 12:
                        ...
                        if(turnPlayer.getHand().isEmpty())
                            break;
                        do{
                            choice = Utils.nextInt(turnPlayer + ": <Choose the card you want to play or 0 to cancel") - 1;
                        ...
                        if(checkPlayable(name, bang)){
                            switch (name) {
                                ...
                                case "Bang!":
                                    ...
                                    do{
                                        printDistances(-1);
                                        choice = Utils.nextInt(turnPlayer + ": <Choose the player you want to shoot or 0 to cancel") - 1;
                                ...
                                case "Cat Balou":
                                    do{
                                        printActivePlayers();
                                        choice = Utils.nextInt(turnPlayer + ": <Choose the player you want to discard a card from or 0 to cancel") - 1;
                                    }while(choice < -1 || choice >= nPlayers);
                                    if(choice == -1)
                                        break;
                                    target = choice;
                                ...
                                case "Jail":
                                    do{
                                        printActivePlayers();
                                        choice = Utils.nextInt(turnPlayer + " <Choose the player you want to put in jail or 0 to cancel") - 1;
                                        if(choice == sheriffIx)
                                ...
                                case "Duel":
                                    do{
                                        printActivePlayers();
                                        choice = Utils.nextInt(turnPlayer + " <Choose a player you want to duel or 0 to cancel") - 1;
                                    }while(choice < -1 || choice >= nPlayers);
                                    if(choice != -1){
                                        turnPlayer.discard(card, DiscardPile);
            ...
            turnPlayer.thirdPhase(DiscardPile);
            if(turnPlayerIx == nPlayers - 1)
                turnPlayerIx = 0;
            else
                turnPlayerIx++;
    ...
    private PlayingCard[] createCards(){
        ...
        playingCards[3] = new PlayingCard("Jail", '♠', 11);
        playingCards[4] = new PlayingCard("Duel", '♠', 11);
        playingCards[5] = new PlayingCard("Mustang", '♥', 9);
        playingCards[6] = new PlayingCard("Barrel", '♠', 12);
        playingCards[7] = new PlayingCard("Jail", '♠', 10);
        playingCards[8] = new PlayingCard("Scope", '♠', 1);
        playingCards[9] = new PlayingCard("Dynamite", '♥', 2);
        playingCards[10] = new PlayingCard("Bang!", '♦', 7);
        playingCards[11] = new Weapon("Remington", '♣', 13, 3, false);
        playingCards[12] = new PlayingCard("Wells Fargo", '♥', 3);
        playingCards[13] = new PlayingCard("Beer", '♥', 10);
        playingCards[14] = new PlayingCard("Bang!", '♣', 6);
        playingCards[15] = new PlayingCard("Bang!", '♣', 5);
        playingCards[16] = new PlayingCard("Beer", '♥', 7);
        playingCards[17] = new PlayingCard("Missed!", '♠', 4);
        playingCards[18] = new PlayingCard("Bang!", '♦', 13);
        playingCards[19] = new PlayingCard("Bang!", '♦', 12);
        playingCards[20] = new PlayingCard("Missed!", '♣', 11);
        playingCards[21] = new Weapon("Winchester", '♠', 8, 5, false);
        playingCards[22] = new PlayingCard("Stagecoach", '♠', 9);
        playingCards[23] = new PlayingCard("Beer", '♥', 6);
        playingCards[24] = new PlayingCard("Bang!", '♦', 4);
        playingCards[25] = new PlayingCard("Gatling", '♥', 10);
        playingCards[26] = new PlayingCard("Panic!", '♦', 8);
        playingCards[27] = new PlayingCard("Bang!", '♥', 13);
        playingCards[28] = new PlayingCard("Panic!", '♥', 12);
        playingCards[29] = new PlayingCard("Jail", '♥', 4);
        playingCards[30] = new PlayingCard("Missed!", '♣', 10);
        playingCards[31] = new Weapon("Volcanic", '♣', 10, 1, true);
        playingCards[32] = new PlayingCard("Bang!", '♣', 4);
        playingCards[33] = new PlayingCard("Missed!", '♣', 1);
        playingCards[34] = new PlayingCard("General Store", '♣', 9);
        playingCards[35] = new PlayingCard("Indians!", '♦', 13);
        playingCards[36] = new PlayingCard("Bang!", '♣', 2);
        playingCards[37] = new PlayingCard("Bang!", '♦', 6);
        playingCards[38] = new PlayingCard("Bang!", '♦', 10);
        playingCards[39] = new PlayingCard("Bang!", '♦', 3);
        playingCards[40] = new PlayingCard("Cat Balou", '♦', 10);
        playingCards[41] = new Weapon("Schofield", '♠', 13, 2, false);
        playingCards[42] = new PlayingCard("Bang!", '♦', 5);
        playingCards[43] = new PlayingCard("Beer", '♥', 8);
        playingCards[44] = new PlayingCard("Beer", '♥', 11);
        playingCards[45] = new PlayingCard("Bang!", '♣', 3);
        playingCards[46] = new PlayingCard("Panic!", '♥', 11);
        playingCards[47] = new PlayingCard("Bang!", '♥', 12);
        playingCards[48] = new PlayingCard("Missed!", '♠', 2);
        playingCards[49] = new PlayingCard("Saloon", '♥', 5);
        playingCards[50] = new PlayingCard("Bang!", '♦', 2);
        playingCards[51] = new Weapon("Schofield", '♣', 12, 2, false);
        playingCards[52] = new PlayingCard("Bang!", '♣', 9);
        playingCards[53] = new PlayingCard("Bang!", '♠', 1);
        playingCards[54] = new PlayingCard("Bang!", '♥', 1);
        playingCards[55] = new PlayingCard("Stagecoach", '♠', 9);
        playingCards[56] = new PlayingCard("Bang!", '♦', 8);
        playingCards[57] = new PlayingCard("Bang!", '♦', 11);
        playingCards[58] = new PlayingCard("Beer", '♥', 9);
        playingCards[59] = new PlayingCard("Duel", '♦', 12);
        playingCards[60] = new PlayingCard("Duel", '♣', 8);
        playingCards[61] = new Weapon("Schofield", '♣', 11, 2, false);
        playingCards[62] = new PlayingCard("Panic!", '♥', 1);
        playingCards[63] = new PlayingCard("Indians!", '♦', 1);
        playingCards[64] = new PlayingCard("Mustang", '♥', 8);
        playingCards[65] = new PlayingCard("Cat Balou", '♦', 9);
        playingCards[66] = new PlayingCard("General Store", '♠', 12);
        playingCards[67] = new PlayingCard("Bang!", '♦', 1);
        playingCards[68] = new PlayingCard("Missed!", '♠', 7);
        playingCards[69] = new PlayingCard("Missed!", '♠', 3);
        playingCards[70] = new PlayingCard("Missed!", '♠', 8);
        playingCards[71] = new Weapon("Volcanic", '♠', 10, 1, true);
        playingCards[72] = new PlayingCard("Missed!", '♣', 12);
        playingCards[73] = new PlayingCard("Cat Balou", '♥', 13);
        playingCards[74] = new PlayingCard("Bang!", '♣', 8);
        playingCards[75] = new PlayingCard("Bang!", '♦', 9);
        playingCards[76] = new PlayingCard("Missed!", '♠', 6);
        playingCards[77] = new PlayingCard("Missed!", '♠', 5);
        playingCards[78] = new PlayingCard("Barrel", '♠', 13);
        playingCards[79] = new PlayingCard("Missed!", '♣', 13);
        return playingCards;
    }
    ...
    private void sidKetchum(String message, LinkedList<PlayingCard> DiscardPile){
        if(sidKetchumIx >= 0 && Players[sidKetchumIx].getHand().size() >= 2){
            ...
            boolean c;
            do{
                c = Players[sidKetchumIx].sidKetchum(DiscardPile);
            }while(c);
        }
    }
    ...
    private boolean bang(int defender, LinkedList<PlayingCard> Deck, LinkedList<PlayingCard> DiscardPile){
        ...
        if(barrelEffect(Players[defender], Deck, DiscardPile))
            shot--;
        if(shot > 0)
            if(Players[defender].jourdonnais(Deck, DiscardPile))
                shot--;
    ...
    private int[] gatling(LinkedList<Integer> Hit, LinkedList<PlayingCard> Deck, LinkedList<PlayingCard> DiscardPile){
        for(int i = turnPlayerIx + 1; i != turnPlayerIx && !(i == nPlayers && turnPlayerIx == 0); i++){
            ...
            if(barrelEffect(Players[i], Deck, DiscardPile))
                continue;
            if(Players[i].jourdonnais(Deck, DiscardPile))
                continue;
    ...
    private int[] multipleHits(LinkedList<Integer> Hit, LinkedList<PlayingCard> DiscardPile){
        ...
        for (int i : Hit) {
            if(Players[i].getLives() == 1){
                if(!Players[i].savingBeer(DiscardPile, nPlayers == 2)){
                    Players[i].subLife();
                    Dead.add(i);
                }
            }
    ...
    private boolean barrelEffect(Player defender, LinkedList<PlayingCard> Deck, LinkedList<PlayingCard> DiscardPile){
        if(defender.isActiveCard("Barrel")){
            int choice;
            do{
                choice = Utils.nextInt(defender + " <Choose 1 to activate your Barrel or 0 to ignore");
            }while(choice < 0 || choice > 1);
            if(choice == 1){
                boolean res = defender.drawHearts(Deck, DiscardPile);
                if(res){
                    System.out.println("Missed!");
                    return true;
                }
    ...
    private boolean dynamite(LinkedList<PlayingCard> Deck, LinkedList<PlayingCard> DiscardPile){
        ...
        if(turnPlayer.getCharacter().getName() == "Lucky Duke"){
            ...
            do{
                choice1 = Utils.nextInt("Lucky Duke: <Choose which one you want to use") - 1;
            }while (choice1 < 0 || choice1 > 1);
            do{
                choice2= Utils.nextInt("Lucky Duke: <Choose which one you want to discard before") - 1;
            ...
            if(tmp[choice1].getSuit() == '♠' && tmp[choice1].getRank() >= 2 && tmp[choice1].getRank() <= 9)
                return turnPlayer.explosion(Deck, DiscardPile, nPlayers == 2);
        }
        else{
            if(Deck.isEmpty())
                Deck.addAll(discardIntoDeck(DiscardPile));
            ...
            if(DiscardPile.getFirst().getSuit() == '♠' && DiscardPile.getFirst().getRank() >= 2 && DiscardPile.getFirst().getRank() <= 9)
                return turnPlayer.explosion(Deck, DiscardPile, nPlayers == 2);
        }
